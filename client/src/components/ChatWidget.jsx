...
const ChatWidget = () => {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end">
            ...
            <>
              <div className="flex-1 overflow-y-auto p-4 bg-gray-50 space-y-3">
                 <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
                    {suggestions.map((s, i) => (
                      <button key={i} onClick={() => sendMessage(s)} className="text-xs bg-white border border-blue-200 text-blue-600 px-3 py-1 rounded-full whitespace-nowrap hover:bg-blue-50"> {s} </button>
                    ))}
                 </div>
              ...
              <div className="p-3 bg-white border-t flex gap-2">
                <input  className="flex-1 outline-none text-sm"  placeholder="Type your query..."  value={input}  onChange={(e) => setInput(e.target.value)}  onKeyPress={(e) => e.key === 'Enter' && sendMessage()} />
      ...
      <button  onClick={() => setIsOpen(!isOpen)} className="bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-full shadow-lg transition-transform hover:scale-105 flex items-center justify-center w-14 h-14" > 
        {isOpen ? <FaTimes size={24} /> : <FaCommentDots size={28} />} </button>
    </div>
  );
};
